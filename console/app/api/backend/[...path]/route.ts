/**
 * BFF 后端代理（allowlist）：
 * - 仅代理固定路径 /v1/*，拒绝其它路径；webhook 入口禁止经 BFF 转发（防止伪造 Shopify 回调）。
...
async function handle(request: NextRequest, paramsPromise: Promise<{ path: string[] }>) {
  ...
  if (!apiPath.startsWith("/v1/") || DENY_PATHS.has(apiPath)) {
    return errorJson(404, "not_found", "BFF 仅代理允许的后端路径（/v1/* 白名单）");
  }
  ...
  if (method !== "GET") {
    ...
    // 期望 Origin：显式 CONSOLE_ORIGIN 优先；未配置时按请求 Host 推导（浏览器 Origin 与 Host 一致）。
    // 注：dev 模式下 Next 内部 request.url 可能解析为 localhost，故不直接使用 URL.origin。
    const selfUrl = new URL(request.url);
    const host = request.headers.get("host");
    const expectedOrigin = process.env.CONSOLE_ORIGIN || (host ? `${selfUrl.protocol}//${host}` : selfUrl.origin);
